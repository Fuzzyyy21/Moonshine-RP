...
| **Ratenlimit** | Netz-Events, die Geld oder Items bewegen, ohne `MS.RateLimit`. Der Handler mag Besitz, Nähe und Kontostand prüfen — ohne Begrenzung lässt er sich trotzdem im Dauerfeuer aufrufen. Zwanzig Handler hatten keine, darunter `factions:server:vaultPut`, während `vaultTake` daneben eine hatte. |
